...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by SWAMP %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"swampaddress\"\n"
            "\nEncrypts a private key corresponding to 'swampaddress'.\n"
            "\nArguments:\n"
            "1. \"swampaddress\"   (string, required) The swamp address for the private key (you must hold the key already)\n"
