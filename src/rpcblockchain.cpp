...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("chaiswamprk", blockindex->nChainWork.GetHex()));
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in swamp\n"
...
UniValue getblockchaininfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"chaiswamprk\": \"xxxx\"     (string) total amount of work in active chain, in hexadecimal\n"
            "}\n"
            "\nExamples:\n" +
    ...
    obj.push_back(Pair("chaiswamprk", chainActive.Tip()->nChainWork.GetHex()));
    return obj;
}
...
UniValue getchaintips(const UniValue& params, bool fHelp)
{
    ...
    /* Build up a list of chain tips.  We start with the list of all
       known blocks, and sswampessively remove blocks that appear as pprev
       of another block.  */
