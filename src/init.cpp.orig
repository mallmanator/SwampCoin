...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "swamp.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "swampd.pid"));
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SWAMP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SWAMP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, proxy, swamp, (obfuscation, swiftx, masternode, mnpayments)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SWAMP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SWAMP specific functionality (Masternodes, Obfuscation, SwiftX) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizeswampamount=<n>", strprintf(_("Keep N SWAMP anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2018 The Bitcoin Core Developers")) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018-2018 The SWAMP Core Developers")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SWAMP is running in a usable environment with all
...
#endif
    // Make sure only a single SWAMP process is using the data directory.
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeUccAmount = GetArg("-anonymizeswampamount", 0);
    ...
    LogPrintf("Anonymize SWAMP Amount %d\n", nAnonymizeUccAmount);
    ...
    /* Denominations
       ...
       For example:
       1SWAMP+1000 == (.1SWAMP+100)*10
       10SWAMP+10000 == (1SWAMP+1000)*10
    */
