...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        bool sswampess = false;
        int count = 0;

        BOOST_FOREACH (const CTxIn item, sigs) {
            if (AddScriptSig(item)) sswampess = true;
...
//
// Unlock coins after Obfuscation fails or sswampeeds
//
...
std::string CObfuscationPool::GetStatus()
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSswampess < minBlockSpacing || !masternodeSync.IsBlockchainSynced()) {
...
void CObfuscationPool::Check()
{
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SSWAMPESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
...
void CObfuscationPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        RelayCompletedTransaction(sessionID, false, MSG_SSWAMPESS);
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.001 SWAMP per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::CheckTimeout()
{
    ...
    if (!fMasterNode) {
        ...
        case POOL_STATUS_ERROR:
            ...
            break;
        case POOL_STATUS_SSWAMPESS:
            LogPrint("obfuscation", "CObfuscationPool::CheckTimeout() -- Pool sswampess -- Running Check()\n");
            Check();
            break;
        }
    }
...
{
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SSWAMPESS) return false;
...
void CObfuscationPool::CompletedTransaction(bool error, int errorID)
{
    ...
    if (error) {
        ...
        LogPrintf("CompletedTransaction -- sswampess \n");
        UpdateState(POOL_STATUS_SSWAMPESS);

        UnlockCoins();
        SetNull();

        // To avoid race conditions, we'll only let DS run once per block
        cachedLastSswampess = chainActive.Tip()->nHeight;
    }
    lastMessage = GetMessageByID(errorID);
}
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSswampess < minBlockSpacing) {
...
bool CObfuscationPool::SendRandomPaymentToSelf()
{
    ...
    bool sswampess = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, strFail, coinControl, ONLY_DENOMINATED);
    if (!sswampess) {
    ...
    LogPrintf("SendRandomPaymentToSelf Sswampess: tx %s\n", wtx.GetHash().GetHex());
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    bool sswampess = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
    ...
    if (!sswampess) {
        ...
        sswampess = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
    ...
    // use the same cachedLastSswampess as for DS mixinx to prevent race
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100SWAMP+1 ( bit on if present )
    // bit 1 - 10SWAMP+1
    // bit 2 - 1SWAMP+1
    // bit 3 - .1SWAMP+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("swamp-obfuscation");
