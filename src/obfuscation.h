...
#define POOL_STATUS_SSWAMPESS 8              // sswampess
...
{
public:
    /// Is the inputs associated with this public key? (and there is deposit SWAMP amount - checking if valid masternode)
    ...
    /// Verify the message, returns true if sswampcessful
...
public:
    ...
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SSWAMPESS)) {
            LogPrint("obfuscation", "CObfuscationPool::UpdateState() - Can't set state to ERROR or SSWAMPESS as a Masternode. \n");
            return;
        }
