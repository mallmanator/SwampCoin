...
#if defined(NDEBUG)
#error "SWAMP cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uswamp) are considered zero fee (for relaying and mining)
...
void static CheckBlockIndex()
{
    ...
    while (pindex != NULL) {
        ...
        assert(pindex->pprev == NULL || pindex->nChainWork >= pindex->pprev->nChainWork);                            // For every block except the genesis block, the chaiswamprk must be larger than the parent's.
